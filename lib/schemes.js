module.exports = (function(core){
  ...
  core.Future = core.Class({
      ...
      init: function(v){
        ...
        this.__complete__ = core.funcs.bind(function(){
          if(this.isCompleted()) return this;
          cargs = core.enums.toArray(arguments);
          this.emit.apply(null,['success'].concat(cargs));
          completed.on();
          return this;
        },this);

        this.__completeError__ = core.funcs.bind(function(e){
          // core.Asserted(core.valids.isInstanceOf(e,Error),'first argument must be an Error object');
          if(this.isCompleted()) return this;
          isError = true;
          cargs = core.enums.toArray(arguments);
          this.emit.apply(null,['error'].concat(cargs));
          completed.on();
          return this;
        },this);

        this.__then__ = core.funcs.bind(function(fn){
          var ise = false, res;
          // if(!core.valids.isFunction(fn)) return;
          if(core.Future.isType(fn)) return this.chain(fn);
          if(this.itSucceeded()){
              try{
                res = fn.apply(null,cargs);
              }catch(e){
                ise = true;
                res = e;
              };
              ...
              var then = core.Future.make();
              if(ise) then.completeError(res);
              else then.complete(res);
              return then;
          }
          else if(this.itErrored()) return this;
      ...
      wait: function(){
        ...
        core.enums.each(args,function(e,i,o,fn){
          ...
          e.then(function(){
            slist = slist.concat(core.enums.toArray(arguments));
            count += 1;
            return fn(null);
          });

          e.onError(function(){
            var ka = core.enums.toArray(arguments);
            elist = elist.concat(ka);
            return fn(ka);
          });
        },function(_,err){
          if(err) return then.completeError.apply(then,err);
          return (count >= total ?  then.complete.apply(then,slist) : null);
        });
  ...
  core.FutureStream = core.Future.extends({
    ...
    chainStream: function(fx){
      ...
      var dbfx = function(){ fx.out().emitEvent.apply(fx.out(),['dataBegin'].concat(arguments)); };
      var defx = function(){ fx.out().emitEvent.apply(fx.out(),['dataEnd'].concat(arguments)); };
  ...
  core.Query = function(target,schema){
    var _ = core;
    ...
    _.funcs.selfReturn(ax,'where',function(q){
      ...
      if(_.valids.isFunction(q)){
        push({'op':'$stream', 'key': q},fid.fetch);
      }
    ...
    _.funcs.selfReturn(ax,'every',function(ms,total){
      if(_.valids.not.isNumber(ms) || (total && _.valids.not.isNumber(total))) return;
      push({'op':'$cycle',key:{'$total': total || Infinity,'$timeout': ms}});
    });
    ...
    // _.funcs.selfReturn(ax,'nstream',function(n,fn){
    //   if(n > ops.length || _.valids.not.isFunction(fn)) return;
    //   var ci = sips[n];
    //   if(!ci) return;
    //   ci.push(fn);
    // });
  ...
  core.QueryStream = function(connection){
    ...
    _.funcs.selfReturn(ax,'where',function(tag,fn,atomic){
      ...
      mix.add(function(d,next,end){
        var q = d.q, sm = d.sx;
        if(q.op.toLowerCase() !== tag) return next();
        return fn.call(connection,d.with,q,sm,q['$schema']);
      });
    });

    _.funcs.selfReturn(ax,'query',function(t){
      if(_.valids.not.isObject(t)) return;
      ...
      _.enums.eachSync(ops,function(e,i,o,fx){
        ...
        if(li.length > 0){
          ...
          inter.binding = inter.in().stream(inter.out());
          sx.chain(inter);
  ...
  */
  ...
  core.Hooks = core.Class({
    init: function(id){
      ...
      this.$secure('distribute',function(){
        return this.distributeWith(this,arguments);
      });
    },
  ...
  core.EventStream = core.Class({
    ...
    after: function(name,fn){
      if(!this.eventSpace[name]) return;
      if(this.fired.indexOf(name) != -1) fn.call(this);
      var es = this.eventSpace[name];
      es.addAfter(fn);
      return this;
    },
    ...
    afterOnce: function(name,fn){
      if(!this.eventSpace[name]) return;
      if(this.fired.indexOf(name) != -1) return fn.call(this);
      var es = this.eventSpace[name];
      es.addAfterOnce(fn);
      return this;
    },
    ...
    hookProxy: function(obj,fn){
      ...
      obj[fn('flushAllEvents')] = this.$bind(this.flushAll);
      obj[fn('flushEvent')] = this.$bind(this.flush);
  ...
  core.Stream = core.Class({
    init: function(){
      ...
      this.emitEvent = core.funcs.bind(this.events.emit,this.events);
