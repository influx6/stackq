module.exports = (function(core){
  ...
  core.StreamSelect = function(shouldRemove,sim){
    shouldRemove = core.valids.isBoolean(shouldRemove) ? shouldRemove : false;
    ...
    select.streams = core.Stream.make();
    select.streams.on(packets.add);
    select.streams.on(function(j){
      asm.resolve(true);
    });
    ...
    var operationGenerator = function(fn){
      ...
      asm.done(function(r){
        ...
        sm.onEvent('drain',function(){ sm.close(); });
    ...
    var proxyemit = core.funcs.bind(select.streams.emit,select.streams);

    var bindStream = function(stream){
      if(!core.Stream.isType(stream)) return;
      stream.on(proxyemit);
      select.streams.onEvent('close',function(){
        stream.offEvent(proxyemit);
      });
    };

    if(sim) bindStream(sim);
    ...
    select.createMux = core.funcs.bind(createMux,select);
    select.bindStream = core.funcs.bind(bindStream,select);
  ...
  core.Stream = core.Class({
    init: function(){
      ...
      core.Util.createProperty(this,'totalDataSubscriber',{
        get: function(){
          return this.events.sizeOf('data');
        }
      });
