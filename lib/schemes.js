module.exports = (function(core){
  ...
  core.Query = function(target,schema){
    ...
    core.Util.createProperty(map,'queryKey',{ get: function(){ return querysig; }});
    ...
    var push = function(q,n){
      q['$schema'] = schema;
      if(n == 1) ads.push(ops.length);
      if(n == 2) sv.push(ops.length);
      if(n == 3) ft.push(ops.length);
      if(n == 4) mu.push(ops.length);
      if(n == 5) ds.push(ops.length);
      sips[ops.length] = [];
      ops.push(q);
    };
  ...
  core.QueryStream = function(connection){
    ...
    _.funcs.selfReturn(ax,'where',function(tag,fn,atomic){
      ...
      mix.add(function(d,next,end){
        ...
        return fn.call(connection,d.with,q,sm,d.init);
      });
    });
    ...
    _.funcs.selfReturn(ax,'query',function(t){
      ...
      _.enums.each(ops,function(e,i,o,fx){
        var sx = _.Stream.make(), cs = _.enums.last(fsm), li = sips[i];
        _.enums.each(li,_.funcs.bindWith(sx));
        mix.emit({'q': e, 'sx': sx, 'with': docs, 'init': cs ? false : true});
        if(cs) binders.push(cs.stream(sx,true));
        fsm.push(sx);
        return fx(null);
      },function(i,err){});
      return (ax.current = {
  ...
  */
  ...
  core.Hooks = core.Class({
    init: function(id){
      this.id = id;
      this.before = core.Distributors();
      this.in = core.Distributors();
      this.after = core.Distributors();
  ...
  core.EventStream = core.Class({
    ...
    events: function(name){
      ...
      this.afterSpace[name] = core.Distributors();
      return this.eventSpace[name];
    },
    afters: function(name){
      return this.afterSpace[name];
    },
    after: function(name,fn){
      if(this.fired.indexOf(name) != -1) return fn.call(null,true);
      var af = this.afters(name);
      if(!af) return;
      af.addOnce(fn);
      if(this.fired.indexOf(name) != -1) af.distribute(true);
    },
    on: function(name,fn){
  ...
  core.Stream = core.Class({
    init: function(){
      ...
      this.mutts = core.Middleware(this.$closure(function(f){
        ...
        this.packets.add(f);
        this.emit('dataCount',this.packets.size());
  ...
  core.Configurable = core.Class({
    init: function(){
      ...
      this.events = core.EventStream.make();
      this.on = this.events.$closure(this.events.on,this.events);
      this.off = this.events.$closure(this.events.off,this.events);
      this.once = this.events.$closure(this.events.once,this.events);
      this.offOnce = this.events.$closure(this.events.offOnce,this.events);
    },
    config: function(map){
      this.configs.addAll(map);
    },
