module.exports = (function(core){
  ...
  streams.Streamable = function(fn){
    ...
    core.events.set('initd');
    core.events.set('drain');
    core.events.set('end');
    core.events.set('close');
    core.events.set('listenerAdded');
    core.events.set('listenerRemoved');
    core.events.set('endListenerAdded');
    core.events.set('endListenerRemoved');

    core.events.on('listenerAdded',function(f){
   ...
   core.states.addState('closed',{
    ...
    util.extends(core,{
      emit: util.bind(function(name,fn){
        this.events.emit(name,fn);
      },core),
      off: util.bind(function(name,fn){
        this.events.off(name,fn);
      },core),
      once: util.bind(function(name,fn){
        this.events.once(name,fn);
      },core),
      on: util.bind(function(name,fn){
        this.events.on(name,fn);
      },core),
	    add: function(n){
	      if(this.closed() || this.locked()) return;
	      this.events.emit('initd',n);
	    ...
	    untellEnd: function(fn){
	      if(this.closed()) return;
        this.events.emit('endListenerRemoved',fn);
      ...
      tellEndOnce: function(fn){
        ...
        this.events.emit('endListenerAdded',fn);
	      return fn;
      },
	    tellEnd: function(fn){
	      if(this.closed()) return;
	    ...
	    untell: function(fn){
	      if(this.closed()) return;
        this.events.emit('listenerRemoved',fn);
      ...
      tellOnce: function(fn){
        ...
        this.events.emit('listenerAdded',fn);
	      return fn;
      },
	    tell: function(fn){
	      if(this.closed()) return;
