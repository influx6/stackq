module.exports = (function(core){
  ...
  */
  ...
  core.Persisto = core.Configurable.extends({
    init: function(){
      ...
      this.linkPersisto = this.$bind(function(stream){
        if(!core.Persisto.isType(stream)) return;
        var it = this.packets.iterator(), data,node;
  ...
  .muxin({
    ...
    copy: function(ps){
      if(core.Stream.isType(ps)){
        var p = this.copyStream(ps);
        this.afterOnce('end',function(){
      ...
      if(core.Persisto.isType(ps)){
        var p = this.copyPersisto(ps);
        this.afterOnce('end',function(){
          if(p && core.valids.Function(p.dropConnection)){ p.dropConnection(); }
        });
        return p;
    ...
    link: function(ps){
      if(core.Stream.isType(ps)){
        var p = this.linkStream(ps);
        return p;
      }
      if(core.Persisto.isType(ps)){
        var p = this.linkPersisto(ps);
        return p;
      }
      return ps;
    },
