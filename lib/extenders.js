 ...
 /* former schemes code */
  ...
  core.Contract = function(n,pickerfn){
    ...
    cd.interogate = function(m,picker){
      ...
      if(valids.isString(handler) && handler == '*'){
        return this.allowed.distribute(m);
      }
      if(valids.isString(handler)){
        if(handler == picker(m)) return this.allowed.distribute(m);
      }
  ...
  core.Class = function(attr,static,_super){
    ...
    var type = core.ClassType(_super,function(){
      ...
      this.__instanceGUUID__ = function(){ return spid; };
    ...
    klass.extends = function(at,st){
      ...
      util.extends(child,st,{
        __childGUUID__: function(){ return childguid; },
      });
      return child;
    };
    ...
    klass.isChild = function(q){
      ...
      if(core.valids.isFunction(q.__childGUUID__)){
        if(children.indexOf(q.__childGUUID__()) != -1) return true;
      }
      return false;
    };

    klass.isInstance = function(kc){
      var iss = kc instanceof klass;
      var isx = (core.valids.isFunction(kc.__instanceGUUID__) ? spid == kc.__instanceGUUID__() : false);
      return iss || isx;
    };
  ...
  */
  ...
  core.Persisto = core.Configurable.extends({
    init: function(){
      ...
      this.patch = this.$bind(function(stream){
        if(!core.Stream.isInstance(stream)) return;
        var it = this.packets.iterator();

        while(it.moveNext()){
          stream.emit(it.current(),it.currentNode());
        };
        if(!this.busy.isOn()) stream.endData();
        else{
      ...
      this.patchto = this.$bind(function(stream){
        if(!core.Persisto.isInstance(stream)) return;
        var it = this.packets.iterator();
        while(it.moveNext()){
          stream.push(it.current());
        };
        this.router.add(stream.$push);
  ...
  .muxin({
    ...
    copy: function(ps){
      var p = this.patchto(ps);
      if(p && core.valids.Function(p.dropConnection)){ p.dropConnection(); }
      return p;
    },
    link: function(ps){
      return this.patchto(ps);
    },
    end: function(){
      this.busy.off();
