module.exports = (function(core){
  ...
  */
  ...
  core.Immutate = core.Configurable.extends({
    ...
    ghost: function(addr){
      return this.cursor.ghost(addr);
    },
    snapshot: function(f){
     return this.cursor.snapshot(f);
    },
    value: function(){
    ...
    transform: function(k){
      if(core.Immutate.instanceBelongs(k)) return k.get();
  ...
  core.ImmutateCursor = core.GatewaySequence.extends({
    ...
    rewind: function(n){
      ...
      if(this.__dimension >= this.length()) return;
    ...
    ghost: function(addr){
      ...
      return core.GhostCursor.make(this,addr);
    },
    snapshot: function(f){
      ...
      if(this.has(first))
        return this.getCore().get(first).snapshot(rest.join('.'));

      return null;
    },
    snapshotValue: function(f){
      var fd = this.snapshot(f);
      if(fd) return fd.value();
      return null;
    },
    value: function(){
      return this.toJS();
