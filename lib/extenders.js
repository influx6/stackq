module.exports = (function(core){
  ...
  */
  ...
  core.SequenceFuture = core.Future.extends({
    ...
    mapobj: function(fn,fc){
     return this.then(function(seq){
        ...
        core.ObjectMapSequence.make(seq,fn,fc,seqf);
        return seqf;
     });
    },
    map: function(fn,fc){
     return this.then(function(seq){
        var seqf = core.SequenceFuture.make(seq);
        core.MapSequence.make(seq,fn,fc,seqf);
        return seqf;
     });
    },
    filter: function(fn,fc){
     return this.then(function(seq){
        var seqf = core.SequenceFuture.make(seq);
        core.FilterSequence.make(seq,fn,fc,seqf);
        return seqf;
     });
    },
    while: function(fn,fc){
     return this.then(function(seq){
        var seqf = core.SequenceFuture.make(seq);
        core.WhileSequence.make(seq,fn,fc,seqf);
        return seqf;
     });
    },
    values: function(){
  ...
  core.OperationalSequence = core.Sequence.extends({
    init: function(parent,eachitem,completion,seqfuture){
      if(core.valids.exists(seqfuture)){
        core.Asserted(core.SequenceFuture.instanceBelongs(seqfuture),'please pass a seqfuture instance');
      }
  ...
  core.MemoizedSequence = core.OperationalSequence.extends({
    init:function(parent,c,c,f){
     this.$super(parent,null,null,f);
  ...
  core.Immutate = core.Configurable.extends({
    ...
    transformMutates: function(ob){
      ...
      var trans = seq.mapobj(function(v,k){
         return core.Immutate.transform(v);
      });
      return trans;
    },
    transform: function(k){
      if(core.Immutate.instanceBelongs(k)) return k.get();
      if(core.valids.Primitive(k)) return core.ImmutateValue.make(k);
      return core.ImmutateCollection.transform(k);
    },
    detransform: function(k){
  ...
  core.ImmutateCollection = core.Immutate.extends({
    init: function(data){
      this.$super(data);
      this.type = 'ob-immutate';
      this.mutates = core.Immutate.transformMutates(data);
    }
  },{
    transform: function(k){
      if(core.valids.List(k)){
        return core.ImmutateList.make(k);
      }
      if(core.valids.Object(k)){
        return core.ImmutateObject.make(k);
      }
    }
  });

  core.ImmutateObject = core.ImmutateCollection.extends({
    init: function(data){
      this.$super(data);
      this.type = 'ob-immutate';
    }
  ...
  core.ImmutateList = core.ImmutateCollection.extends({
    init: function(data){
      this.$super(data);
      this.type = 'ls-immutate';
    }
  });

  core.ImmutateValue = core.Immutate.extends({
    init: function(data){
      this.$super({value: data});
      this.type = 'val-immutate';
    },
    value: function(){
  ...
  core.ImmuateCursor = core.TargetSequence.extends({
    init: function(im){
