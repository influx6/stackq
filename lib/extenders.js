module.exports = (function(core){
 ...
 /* former schemes code */
  ...
  core.Class = function(attr,static,_super){
    ...
    klass.isChildInstance = function(q){
    ...
    klass.instanceBelongs = function(q){
      return klass.isChildInstance(q) || klass.isInstance(q);
    };
  ...
  */
  ...
  core.Persisto = core.Configurable.extends({
    init: function(){
      ...
      this.linkStream = this.$bind(function(stream){
        console.log('linking stream',core.Stream.isChildInstance(stream),stream);
        if(!core.Stream.isChildInstance(stream)) return;
        ...
        console.log('empty->',this.packets.isEmpty());
      ...
      this.linkPersisto = this.$bind(function(stream){
        console.log('to link',stream);
        if(!core.Persisto.isChildInstance(stream)) return;
        console.log('linking:',stream);
  ...
  .muxin({
    ...
    copy: function(ps){
      if(core.Stream.isChildInstance(ps)){ return this.copyStream(ps); }
      if(core.Persisto.isChildInstance(ps)){ return this.copyPersisto(ps); }
      return ps;
    },
    link: function(ps){
      if(core.Stream.isChildInstance(ps)){ return this.linkStream(ps); }
      if(core.Persisto.isChildInstance(ps)){ return this.linkPersisto(ps); }
      return ps;
    },
    end: function(){
      this.busy.off();
