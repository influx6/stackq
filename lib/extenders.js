module.exports = (function(core){
  ...
  */
  ...
  core.ImmutateHistory = core.Class({
    init: function(im){
      core.Asserted(core.Immutate.instanceBelongs(im),'only for immutate instance or childs');
      this.history = [];
      this.history.push(im);
    },
    current: function(f){
      return this.get(f,core.enums.last(this.history));
    },
    push: function(f){
      if(!core.Immutate.instanceBelongs(f)) return;
      return this.history.push(f);
    },
    flush: function(){
      this.history.length = 0;
    },
    get: function(ind,noSet){
      return this.history[ind] || noSet;
    },
    index: function(cm){
      return this.history.indexOf(cm);
    },
    blastBy: function(f){
      if(core.valids.not.Number(f)) return;
    ...
    blast: function(){
      return this.blastBy(this.history.length - 1);
    }
  });
  ...
  core.Immutate = core.Configurable.extends({
      ...
      mutate: function(fn){
        if(core.valids.not.Function(fn)) return;
        return fn.call(this,this.__obj);
      },
      mutation: function(f){
        if(core.enums.pluckIn(this.__obj,f,'NILL') === 'NILL') return;
        var self = this,cm = core.Immutate.value(this,true);
        cm.mutate(function(ob){
          if(core.valids.Function(f)) f.call(this,ob);
      ...
      get: function(f,noSet){
        if(core.valids.not.String(f)) return;

        f = f.replace(allspaces,'');

        var cursor = this.__cursorCache[f];
        if(core.valids.exists(cursor)) return cursor;

        if(!this.has(f)) return;
  
        cursor = core.ImmutateCursor.make(this,f);
        
        cursor.afterOnce('dead',this.$bind(function(){
          delete this.__cursorCache[f];
        }));

        this.__cursorCache[f] = cursor;
        return cursor;
      },
      has: function(f,noSet){
        if(core.valids.not.String(f)) return false;
        if(core.valids.contains(this.__addrCache,f)) 
          return this.__addrCache[f];
        //check if we asking for the root with ''
        if(f.replace(allspaces,'') === '') return true;
        var find =  core.enums.pluckIn(this.__obj, f.split('.'));
        var state = core.valids.exists(find);
        if(state){ this.__addrCache[f] = find; };
        return state;
      },
      iterator: function(fn,conf,zone){
        if(core.valids.not.Function(fn)) return null;
        conf = core.valids.isObject(conf) ? conf : {};
        var hnx = core.enums.nextIterator(this.__obj,fn,
      ...
      toJS: function(fc,getKeys){
        var self = this,
        keys = [],
        im = core.Immutate.clone(this.__obj,function(e,i){
          keys.push(i);
        },{
          complete: fc,
          zone: self,
      ...
      toObject: function(f){
        if(core.valids.Function(f)) f.call(this,this.__obj);
        return this.__obj;
      },
      toJSON: function(f){
        var json = JSON.stringify(this.__obj);
        if(core.valids.Function(f)) f.call(this,json);
      },
      values: function(f){
        var vals = core.enums.values(this.__obj);
        if(core.valids.Function(f)) f.call(this,this.__obj);
      }
    },
    {
      clone: function(obj,fn,conf,zone){
        ...
        var complete = function(a,err){
          if(core.valids.not.Function(conf.complete)) return;
          return conf.complete.call(this,map,a,err);
        }

        var it = core.enums.nextIterator(obj,function(e,i,o,fx){
          if(core.valids.Function(fn)) fn.call(this,e,i);
          ...
          if(core.ImmutateCursor.instanceBelongs(e) && !e.locked()){
            delete o[i];
            fx(null);
            return it.next();
          };
          ...
          if(core.valids.Object(e) || core.valids.List(e)){
            var fk = core.Immutate.clone(e,null,{ 
              complete: function(m){
                map[i] = m;
                fx(null);
                return it.next();
  ...
  core.ImmutateList = core.Immutate.extends({
    init: function(obj){
      this.$super();
      if(core.ImmutateList.instanceBelongs(obj)){
        this.keys = function(){ return []; };
        this.length = function(){ return 0; };
        this.__obj = obj.toJS(function(map,keys){
          this.keys = function(){ return keys; };
          this.length = function(){ return keys.length; };
        });
      }
  ...
  core.ImmutateMap = core.Immutate.extends({
    init: function(obj){
      this.$super();
      if(core.ImmutateMap.instanceBelongs(obj)){
        this.keys = function(){ return []; };
        this.length = function(){ return 0; };
        this.__obj = obj.toJS(function(map,keys){
          this.keys = function(){ return keys; };
      ...
      if(core.valids.Object(obj)){
        this.__obj = obj;
        var keys = null;
        this.keys = this.$bind(function(){
          if(!!keys) return keys;
          return keys = (Object.keys ? Object.keys(obj) : core.enums.keys(obj));
        });
  ...
  core.ImmutateCursor = core.Configurable.extends({
    init: function(im,addrs){
      core.Asserted(core.Immutate.instanceBelongs(im),'only for immutate instance or childs');
      this.$super();
      this.addr = addrs;
      this.history = core.ImmutateHistory.make(im);

      var addr = addrs.split('.'),
      frozen = core.Switch(),
      locked = core.Switch(),
      isRoot = (addr.length === 1 && addr[0] === ''),
      curIM = null,
      curVal = null;
      ...
      this.$secure('freeze',function(){
        im.offAfter('mutation',this.changeRegulator);
        frozen.on();
      });

      this.$secure('$switchLock',function(){
        locked.off();
      });

      this.isFrozen = this.$bind(function(){ return frozen.isOn(); });
      this.$secure('locked',function(){ return locked.isOn(); });

      this.$secure('core',function(){
        return this.history.current();
      });

      this.$unsecure('callOwner',function(fn){
        if(valids.not.Function(fn)) return;
        return fn.call(this,curVal,addr,this.core());
      });

      this.$secure('changeRegulator',function(f){
        if(this.isFrozen()) return;
        if(core.Immutate.instanceBelongs(f)){
          locked.off();
          var res = this.bindFocusRegulator(f,addr);
          if(!!res.state){
            curVal = res;
            this.history.push(f);
            f.afterOnce('mutation',this.changeRegulator);
            locked.on();
          }else{
            this.emit('dead',res,this);
          }
        }
      });

      this.$secure('bindFocusRegulator',function(f,path){
         if(isRoot){
           return {
             key: null, 
             state: true, 
         ...
         var tree = core.enums.pluckTree(f.__obj,path,noValue);
         if(core.valids.is(tree,noValue)) return { state: false };
         var key = core.enums.last(path);
         var val = valids.exists(key) ? tree[key] : tree;
         return {
           state: true, 
           prime: core.valids.isPrimitive(val),
           key: key, 
           tree: tree, 
         };
       });

       this.$unsecure('get',function(f){
         if(core.valids.not.String(f)) return;
         f = f.replace(/\s+/,'');
         var newaddr = addr.concat(f.split('.'));
         return this.core().get(newaddr.join('.'));
       });
  
      this.changeRegulator(im);
    },
    toJSON: function(f){
      if(!this.locked()){
        if(core.valids.Function(f)) return f('{}');
      }
    ...
    value: function(f){
      ...
      return this.callOwner(function(val,addr,owner){
        var vak;
        if(core.valids.exists(val.key)){
          vak = core.enums.getItem(val.tree,val.key);
        }else{
          vak = val.tree;
        }
        
        if(core.ImmutateCursor.instanceBelongs(vak)) return vak.value();
        return vak;
      });
    },
  }).muxin({
    set: function(f,v){
      ...
      var args = core.enums.toArray(arguments),
      fval = core.enums.first(args), 
      lval = core.enums.last(args);

      return this.callOwner(function(val,addr,owner){
        var mut = owner.mutation(this.$bind(function(ob){
           if(val.prime && args.length > 1){
             return;
           }

           var tree = core.enums.pluckTree(ob,addr,noValue);
          ...
          if(core.ImmutateCursor.instanceBelongs(tree[val.key])){

          }else{
           var root = tree[val.key];
           console.log('setting root with',root);
           if(core.valids.not.exists(root)) return this.$switchLock();
