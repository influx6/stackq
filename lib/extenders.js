 ...
 /* former schemes code */
  ...
  core.GatewaySequence = core.Sequence.extends({
    init: function(co){
      ...
      this.$unsecure('exposeBy',function(fx){
        if(core.valids.exists(this.getCore()) && core.valids.Function(fx)) 
          return fx.call(this,this.getCore());
      });
    },
    getCore: function(){
      return this.core;
  ...
  core.Immutate = core.Configurable.extends({
    ...
    transformMutatesSequence: function(seq,isList){
      ...
      var trans = function(v,k){
         return core.Immutate.transform(v).ghost('');
      };
    ...
    detransformMutates: function(seq,isList){
      ...
      var detrans = function(v,k){
        if(core.Immutate.instanceBelongs(v)) return v.snapshot().toJS();
        if(core.GhostCursor.instanceBelongs(v)) return v.toJS();
  ...
  core.ImmutateCursor = core.GatewaySequence.extends({
    init: function(im,initialData,rootCursor){
      ...
      // var hash = Math.floor(0x435f232 * Math.random(Math.random(50)));
      // this.hash = hash;
  ...
  core.GhostCursor = core.GatewaySequence.extends({
    ...
    isValueCursor: function(){ 
      ...
      return this.exposeBy(function(seq){
        return seq.isValueCursor(); 
      });
    },
    isListCursor: function(){ 
      // if(this.dead.isOn()) return;
      return this.exposeBy(function(seq){
        return seq.isListCursor(); 
      });
    },
    isObjectCursor: function(){ 
      // if(this.dead.isOn()) return;
      return this.exposeBy(function(seq){
        return seq.isObjectCursor(); 
      });
    },
    rewind: function(n){
      // if(this.dead.isOn()) return;
    ...
    toJS: function(){ 
      ...
      var args = core.enums.toArray(arguments);
      return this.exposeBy(function(seq){
        return seq.toJS.apply(seq,args);
      });
    },
    hasAll: function(f){
      // if(this.dead.isOn()) return;
      var args = core.enums.toArray(arguments);
      return this.exposeBy(function(seq){
        return seq.hasAll.apply(seq,args);
      });
    },
    has: function(f){
      // if(this.dead.isOn()) return;
      var args = core.enums.toArray(arguments);
      return this.exposeBy(function(seq){
        return seq.has.apply(seq,args);
      });
    },
    ghost: function(addr,fn){
      // if(this.dead.isOn()) return;
      if(addr === this.addr || addr === '' || core.valids.not.exists(addr)) return this;
      var args = core.enums.toArray(arguments);
      return this.exposeBy(function(seq){
        return seq.ghost.apply(seq,args);
      });
    },
    snapshot: function(f,fn){
      // if(this.dead.isOn()) return;
      var args = core.enums.toArray(arguments);
      return this.exposeBy(function(seq){
        return seq.snapshot.apply(seq,args);
      });
    },
    snapshotValue: function(f,fx){
      // if(this.dead.isOn()) return;
      var args = core.enums.toArray(arguments);
      return this.exposeBy(function(seq){
        return seq.snapshotValue.apply(seq,args);
      });
    },
    value: function(){
      // if(this.dead.isOn()) return;
      var args = core.enums.toArray(arguments);
      return this.exposeBy(function(seq){
        return seq.value.apply(seq,args);
      });
    },
    newSequence: function(fx){
      // if(this.dead.isOn()) return;
      var args = core.enums.toArray(arguments);
      return this.exposeBy(function(seq){
        return seq.newSequence.apply(seq,args);
      });
    },
  });
