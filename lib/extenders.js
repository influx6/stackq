  ...
  */
  ...
  core.Immutate = core.Configurable.extends({
      init: function(obj,ifClone,wait){
        ...
        this.pub('mutation');
        this.__addrCache = {};
        if(core.Immutate.instanceBelongs(obj)){
          this.__obj = obj.toJS();
          this.keys = core.enums.deepClone(obj.keys());
        ...
        // this.coreHistory = core.ImmutateHistory.make(this);
      },
      mutate: function(fn){
        if(core.valids.not.Function(fn)) return;
      ...
      mutation: function(f){
        ...
        var self = this,cm = core.Immutate.value(this.__obj,true);
      ...
      get: function(f,noSet){
        if((arguments.length <= 0 || f == 'value') && !!this.isPrimitive){
          if(core.valids.exists(this.__valueCursor)) return this.__valueCursor;
          this.__valueCursor = ImmuateCursor.make(this,'value');
          return this.__valueCursor;
        }
        return core.ImmutateCursor.make(this,f);
      },
      has: function(f,noSet){
        if(core.valids.not.String(f)) return false;
        if(core.valids.contains(this.__addrCache,f)) return this.__addrCache[f];
      ...
      toJS: function(fc){
        var map = {};
        var it = this.async(function(e,i,o){
          ...
          map[i] = core.enums.deepClone(e);
          return it.next();
        },{ complete: function(){
            if(core.valids.not.Function(fc)) return;
            return fc.call(this,map);
        }});
        it.next();
        return map;
      },
  ...
  core.ImmutateCursor = core.Configurable.extends({
    init: function(im,addrs){
      ...
      this.$secure('callOwner',function(fn){
      ...
      this.$secure('bindFocusRegulator',function(f,path){
         ...
         var key = path.length > 1 ? core.enums.last(path) : null;
         return {
           prime: core.valids.isPrimitive(tree[key]),
           state: true, 
           key: key, 
           tree: tree, 
         };
    ...
    value: function(f){
      ...
      return this.callOwner(function(val,addr,owner){
        if(core.valids.exists(val.key)){
          return core.enums.getItem(val.tree,val.key);
        }
        return val.tree;
      });
    },
    ...
    set: function(f,v){
      ...
      return this.callOwner(function(val,addr,owner){
        if(val.prime){
          ...
          var mut = owner.mutation(this.$bind(function(ob,zf){
             if(core.valids.exists(val.key)){
               var tree = core.enums.pluckTree(f.__obj,path,noValue);
               var nval = core.enums.first(args);
               // if(core.Immutate.instanceBelongs(nval)){
               // }
               tree[core.enums.last(addr)] = nval;
             }
          }));
          return;
        };
        var mut = owner.mutation(this.$bind(function(ob){
           console.log('patching mutation...');
           console.log('vals:',val);
           var tree = core.enums.pluckTree(ob,addr,noValue);
           console.log('tree:',tree);
           if(core.valids.exists(val.key)){
             console.log('key',val.key,ob);
             var tree = core.enums.pluckTree(ob,path,noValue);
             // if(core.Immutate.instanceBelongs(tree[val.key])){
             //    
             // };
